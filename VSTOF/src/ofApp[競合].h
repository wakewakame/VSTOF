...
struct frame {
	...
	bool lock_length; //各子フレームの長さ(mode=0なら縦幅,mode=1なら横幅)の固定on/off
};
//パラメーター値構造体
...
public:
	//関数宣言
	void add(frame *parent, frame *self, std::string name, POINT size,bool mode, int gap, int length, bool lock_length) { //フレームの追加
		...
		self->lock_length = lock_length; //現在の自フレームの長さ(mode = 0なら縦幅, mode = 1なら横幅)の固定のon/off
		self->index = 0;
	...
	bool MakeFramePos(frame *f) { //フレーム初期位置算出
		GetFrameHight(f);
		if (f->num_child != 0) {
			if (f->mode) {
				//横並びの場合
				//可変幅フレームの使用可能領域取得
				int use_length1 = f->length - f->gap * (f->num_child + 1); //初期のフレーム幅からgap分の幅を引く
				int use_length2 = f->size.x - f->gap * (f->num_child + 1); //現在のフレーム幅からgap分の幅を引く
				for (int i = 0; i < f->num_child; i++) {
					use_length1 -= f->childs[i]->length * f->childs[i]->lock_length; //初期のフレーム幅から固定フレーム分の幅を引く
					use_length2 -= f->childs[i]->size.x * f->childs[i]->lock_length; //現在のフレーム幅から固定フレーム分の幅を引く
				}
				for (int i = 0; i < f->num_child; i++) {
					if (f->childs[i]->lock_length) {
						if (i == 0) {
							f->childs[i]->pos.left = f->pos.left + f->gap;
						}else{
							f->childs[i]->pos.left = f->childs[i - 1]->pos.right + f->gap;
						}
						f->childs[i]->pos.right = f->childs[i]->pos.left + f->childs[i]->length;
					}else {
						f->childs[i]->pos.top = f->pos.top + f->gap;
						f->childs[i]->pos.bottom = f->pos.bottom - f->gap;
						int length = utility::Proportion(
							double(f->childs[i]->length),
							0.0,
							double(use_length1),
							0.0,
							double(use_length2)
						...
						f->childs[i]->pos.right = f->childs[i]->pos.left + length;
						f->childs[i]->pos.top = f->pos.top + f->gap;
						f->childs[i]->pos.bottom = f->childs[i]->size.y;
					}
					if (f->childs[i]->num_child != 0) {
						MakeFramePos(f->childs[i]);
					}
				}
			}else{
				//縦並びの場合
				//可変幅フレームの使用可能領域取得
				int use_length1 = f->length - f->gap * (f->num_child + 1); //初期のフレーム幅からgap分の幅を引く
				int use_length2 = f->size.y - f->gap * (f->num_child + 1); //現在のフレーム幅からgap分の幅を引く
				for (int i = 0; i < f->num_child; i++) {
					use_length1 -= f->childs[i]->length * f->childs[i]->lock_length; //初期のフレーム幅から固定フレーム分の幅を引く
					use_length2 -= f->childs[i]->size.y * f->childs[i]->lock_length; //現在のフレーム幅から固定フレーム分の幅を引く
				}
				for (int i = 0; i < f->num_child; i++) {
					if (f->childs[i]->lock_length) {
						if (i == 0) {
							...
							f->childs[i]->pos.top = f->childs[i - 1]->pos.bottom + f->gap;
						}
						f->childs[i]->pos.bottom = f->childs[i]->pos.top + f->childs[i]->length;
					}else {
						f->childs[i]->pos.top = f->pos.top + f->gap;
						f->childs[i]->pos.bottom = f->pos.bottom - f->gap;
						int length = utility::Proportion(
						...
						f->childs[i]->pos.bottom = f->childs[i]->pos.top + length;
						f->childs[i]->pos.left = f->pos.left + f->gap;
						f->childs[i]->pos.right = f->childs[i]->size.x;
					}
					if (f->childs[i]->num_child != 0) {
						MakeFramePos(f->childs[i]);
					}
